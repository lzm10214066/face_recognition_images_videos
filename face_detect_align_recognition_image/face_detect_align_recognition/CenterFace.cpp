...
CenterFace::CenterFace(const std::string &model_file, const std::string &trained_file):vector_size(512)
{
	...
	boost::shared_ptr<Layer<float>> layer = net_->layer_by_name("cosine_scale");
	vector<boost::shared_ptr<Blob<float>>> blobs = layer->blobs();

	
	const float* begin= blobs[0]->cpu_data();
	const float* end = blobs[0]->cpu_data()+blobs[0]->count();

	std::cout << blobs.size() << std::endl;
	std::cout << blobs[0]->count() << std::endl;
	vector<int> shape = blobs[0]->shape();
	//net_ = net;
}
...
vector<float> CenterFace::Predict(const cv::Mat& img)
{
	...
	//Blob<float>* input_label = net->input_blobs()[1];
	...
	shared_ptr<Blob<float>> output_layer = net->blob_by_name("prob");
...
vector<float> CenterFace::extractBlobData(const string &blobName)
{
	std::shared_ptr<Net<float>> net = net_;

	shared_ptr<Blob<float>> output_layer = net->blob_by_name(blobName);
	const float* begin = output_layer->cpu_data();
	const float* end = begin + output_layer->channels();
	return std::vector<float>(begin, end);
}
...
vector<vector<float>> CenterFace::Predict(const std::vector<cv::Mat> imgs)
{
	...
	shared_ptr<Blob<float>> output_layer = net->blob_by_name("fc5");
	const float* begin = output_layer->cpu_data();
	const float* end = begin + output_layer->channels();
