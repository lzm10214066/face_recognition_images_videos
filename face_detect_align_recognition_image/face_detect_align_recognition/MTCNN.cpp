...
Mat MTCNN::image_face_detection_align(const Mat &img)
{
	...
	Mat res;
	if (faces.size()==0) return res;
	else
	{
		double cen_imgx = img.cols / 2.;
		double cen_imgy = img.rows / 2.;
		//vector<int> index(faces.size());
		//for (int i = 0; i < index.size(); ++i) index[i] = i;
		//sort(index.begin(), index.end(), [&](int i, int j) {
		//	double cenx = faces[i].x + faces[i].width / 2.;
		//	double ceny = faces[i].y + faces[i].height / 2.;
		//	double d1 = (cenx - cen_imgx)*(cenx - cen_imgx);

		//	cenx = faces[j].x + faces[j].width / 2.;
		//	ceny = faces[j].y + faces[j].height / 2.;
		//	double d2 = (ceny - cen_imgy)*(ceny - cen_imgy);
		//	//return faces[i].area() > faces[j].area();
		//	return d1 < d2;
		//});
		int index = 0;
		double min_d = img.cols*img.rows;
		for (int i = 0; i < faces.size(); ++i)
		{
			double cenx = faces[i].x + faces[i].width / 2.;
			double ceny = faces[i].y + faces[i].height / 2.;

		    double d=sqrt((cenx - cen_imgx)*(cenx - cen_imgx)+ (ceny - cen_imgy)*(ceny - cen_imgy));
			if (d < min_d)
			{
				min_d=d;
				index = i;
		...
		Mat img_aligned;
		AlignFace aligner;
		aligner.similarity_transform(img, alignments[index], img_aligned);
		return img_aligned;
	}
}

void MTCNN::image_face_detection_align(const Mat &img, vector<Mat> &aligned_faces)
{
	vector<Rect> faces;
	vector<vector<Point2f>> alignments;
	detection(img, faces, alignments);
	...
	for (int i = 0; i < alignments.size(); ++i)
	{
		...
		aligned_faces.push_back(img_aligned);
	}
}

bool MTCNN::is_frontalFace(const Rect &box, const vector<Point2f> &landermarks)
{
...
Mat MTCNN::detectFrontalFace(const Mat &img)
{
	vector<Rect> faces;
	vector<vector<Point2f>> alignments;
	detection(img, faces, alignments);
	...
	//vector<int> index(faces.size());
	//for (int i = 0; i < index.size(); ++i) index[i] = i;
	//sort(index.begin(), index.end(), [&](int i, int j) {return faces[i].area() > faces[j].area(); });

	vector<cv::Point2f> landmarks = alignments[0];
	if (is_frontalFace(Rect(), landmarks) == false) return res;
