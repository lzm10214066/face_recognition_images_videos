...
ImageFace::ImageFace() : dist_threshold(0.8),th_person(0.0)
{
	...
	string centerFace_trained_file = "model/centerFace/face_iter_40000.caffemodel";

	centerFace_model_file="coco/coco_casia_resnet_34.prototxt";
	centerFace_trained_file = "coco/face_train_test_iter_20000.caffemodel";
...
void ImageFace::compareImages(const string &folder_1, const string &folder_2)
{
	...
	for (int i = 0; i != n_faces_1; ++i)
	{
		for (int j = 0; j != n_faces_2; ++j)
		{
			...
			//printf("  %1.4f  ", cos_dist);
			/*Mat vi(feature_faces_1[i]);
			Mat vj(feature_faces_2[j]);
			Mat v_dist = vi - vj;
			double dist = norm(v_dist);*/
			//cout << "dist: " << dist << endl;
			if (cos_dist > 0.5)
...
vector<FaceResult> ImageFace::faceClassification(const string &img_path, int k)
{
	...
	vector<float> x = center_face->extractBlobData("fc5");
	double no = norm(x);
	for (auto &p : prob)
	{
		p /= no;
...
vector<FaceResult> ImageFace::faceClassification(const Mat &aligned_face, int k)
{
	vector<float> prob;
	center_face->extractFeature(aligned_face, prob);
	vector<pair<float, int>> score_k;
	for (int i = 0; i < prob.size(); ++i)
	{
		score_k.push_back(make_pair(prob[i], i));
...
void ImageFace::faceHardMining(const string &actors_folder,const string &out_folder)
{

	vector<string> sub_folders = getFiles(actors_folder, true);
	vector<string> actors_names = getFiles(actors_folder, false);
	int total_val = 0;
	int total_det = 0;
	for (int i = 0; i < sub_folders.size(); ++i)
	{
		string name = actors_names[i];
		cout << name << endl;
		vector<string> img_paths = getFiles(sub_folders[i], true);
		for (auto p : img_paths)
		{
			Mat image = imread(p);
			if (!image.data) {
			...
			vector<Mat> aligned_faces;
			mtcnn->image_face_detection_align(image, aligned_faces);
			if (aligned_faces.empty()) continue;
			for (int j=0;j<aligned_faces.size();++j)
			{
				Mat f = aligned_faces[j];
				vector<FaceResult> temp = faceClassification(f, 1);
				if (temp.empty()) continue;
				if (temp[0].similarity >= 0.0)
				{
					string img_name = getFileName(p);
					string dst_name = out_folder + "/" + name+"_"+img_name + "_" + to_string(j) + ".png";
					imwrite(dst_name, f);
					total_val++;
				}
				total_det++;
	...
	cout << "total_det: " << total_det << endl;
}

void ImageFace::faceWashing(const string &actors_folder)
{
	...
	for (int i = 0; i < sub_folders.size(); ++i)
	{
		...
		for (auto p : img_paths)
		{
			vector<FaceResult> temp = faceClassification(p, 1);
			if (temp.empty())
				cout << p << "  can not detect any face" << endl;
			else if (temp[0].name != name && temp[0].similarity > 0.9)
				cout << p << "  " << "not the person" << endl;
			else
				cout << p << "  left" << endl;
		}
	}
	...
	Ptr<ImageFace> &pt)
{
	for (int i = s; i < s + t; ++i)
	{
		...
		for (int j = 0; j<filePaths.size(); ++j)
		{
			...
			string dst_name = person_folder_out + "/" + personName +"_"+to_string(j)+ ".png";
