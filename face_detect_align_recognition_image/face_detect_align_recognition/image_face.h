...
public:
	...
	vector<FaceResult> faceClassification(const Mat &img, int k);
	vector<vector<FaceResult>> faceClassification_folder(const string &folder, int k);
	vector<string> actors_names;

	void faceWashing(const string &folder);
	void faceHardMining(const string &actors_folder, const string &out_folder);
private:
