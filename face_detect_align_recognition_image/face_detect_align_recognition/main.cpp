...
int main()
{
	...
	for(int i=113;i<lines.size();++i)
	{
		...
		if (t.size() == 3)
		{
			...
			if (dist < 0.4) cout << "i:  " << i << "dist: "<<dist<<endl;
...
int main()
{
	...
	{
		cout << "face wash in folder" << endl;
		string folder = "wash";
		image_facer.faceWashing(folder);
	}
	else if (choose == 8)
	{
		cout << "face wash in folder" << endl;
		string folder = "wash";
		string out = "tt_out";
		image_facer.faceHardMining(folder, out);
	}
