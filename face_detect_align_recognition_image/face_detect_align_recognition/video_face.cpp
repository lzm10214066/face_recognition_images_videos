...
VideoFace::VideoFace():_frame_num_limite(7500), _face_num_limite(50), _step(10), dist_threshold(0.8), num_face_reserve(5),sample_num(10)
{
...
void VideoFace::video_face_detection_for_verification(const string &video_path, vector<Mat> &face_imgs, vector<vector<Point2f>> &alignments)
{
	Mat image;
	VideoCapture cap(video_path);
	if (!cap.isOpened()) {
	...
	long totalFrameNumber = cap.get(CV_CAP_PROP_FRAME_COUNT);
	int total_step = totalFrameNumber / sample_num;
	long frameToStart = total_step / 2;
	...
	while (frame_count <= frame_num_limite) {
		...
		//mtcnn->img_show(image);
		bool have_face = faces.size() > 0;
		if (!have_face || have_face_count >= 3) {
			step = total_step; have_face_count = 0;
		}
		else
		{
			have_face_count++;
			if (have_face_count >= 3) face_detect_count++;
...
void VideoFace::videos_vertification(const vector<string> &video_paths)
{
	...
	for (int i = 0; i < video_paths.size(); ++i)
	{
		...
		vector<Mat> raw_faces;
		vector<vector<Point2f>> alignments;
		video_face_detection_for_verification(temp_path, raw_faces, alignments);
