...
bool MTCNN::is_frontalFace(const Rect &box, const vector<Point2f> &landermarks)
{
	...
	if (dist_l<0 || dist_r<0 || r_d>2.5 || r_d< 1./2.5) return false;
...
void MTCNN::detectFrontalFace(const Mat &img, vector<Rect> &rectangles, vector<std::vector<cv::Point2f>>& alignment)
{
	detection(img);
	for (auto &face : face_total)
	{
		Rect rect = Rect(round(face.bounding_box.y1), round(face.bounding_box.x1),
			round(face.bounding_box.y2) - round(face.bounding_box.y1) + 1, round(face.bounding_box.x2) - round(face.bounding_box.x1) + 1);
		double r = double(rect.area()) / img.size().area();
		if (r < 0.006) continue;
		vector<cv::Point2f> landmarks = face.landmarks;
		if (is_frontalFace(rect, landmarks) == false) continue;
		rectangles.push_back(rect);
		alignment.push_back(landmarks);
	}
	if (rectangles.size() > 2)
	{
		sort(rectangles.begin(), rectangles.end(), [](Rect a, Rect b) {return a.area() > b.area(); });
		rectangles.erase(rectangles.begin() + 2, rectangles.end());
	}
